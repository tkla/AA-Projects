...
function fileFinder(directories, targetFile) {
   ...
   for (item in directories){
      if (directories[item] === null){ // Is a file
         if (item === targetFile){
            res = true;
            return res;
         }
      } else {
         res = fileFinder(directories[item], targetFile);
      }
      if (res) return res;
   }
   return res;
...
function pathFinder(directories, targetFile) {
   if (Object.keys(directories).length === 0) return null; 

   let path = '';
   for (item in directories){
      path = item;
      if (directories[item] === null){ 
         if (item === targetFile){
            return "/" + path;
         }
      } else {
         let tmp = pathFinder(directories[item], targetFile);
         if (tmp !== null) return path + tmp;
      }
      path = null;
   }
   return path;
